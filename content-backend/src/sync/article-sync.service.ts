...
export class ArticleSyncService {
   ...
   * 实际执行同步逻辑（内部方法）
   */
  private async doSyncArticleToFeishu(
    article: Article,
    tenant: Tenant,
  ): Promise<SyncResult> {
    try {
      ...
      if (!article.feishuRecordId) {
        ...
        const freshArticle = await this.articleRepository.findOne({
          where: { id: article.id },
          select: ['feishuRecordId'],
        });
        if (freshArticle && freshArticle.feishuRecordId) {
          article.feishuRecordId = freshArticle.feishuRecordId;
          this.logger.log(
            `🔗 发现已存在的 Feishu Record ID: ${article.feishuRecordId}`,
          );
        }
      }
      ...
      if (article.feishuRecordId) {
        // 更新现有记录
        this.logger.log(
          `更新飞书文章记录: ${article.title} (${article.feishuRecordId})`,
        );
  ...
  async syncArticleFromFeishu(
    ...
    try {
      ...
      const fields = recordRes.data.record
        .fields as Partial<FeishuArticleFields>;
      ...
      if (
        article.updatedAt &&
        feishuUpdatedAt &&
        article.updatedAt.getTime() > feishuUpdatedAt
      ) {
      ...
      // 同步人员字段
      if (fields['内容策划'])
        article.planners = await this.mapFeishuToUsers(fields['内容策划']);
      if (fields['文案撰稿'])
        article.copywriters = await this.mapFeishuToUsers(fields['文案撰稿']);
      if (fields['文章编辑'])
        article.editors = await this.mapFeishuToUsers(fields['文章编辑']);
   ...
   * 批量映射多个用户组 (减少数据库查询)
   */
  private async mapMultipleUserGroups(groups: {
    planners: string[];
    copywriters: string[];
    editors: string[];
  }): Promise<UserGroupMapping> {
    const allUserIds = Array.from(
      new Set([...groups.planners, ...groups.copywriters, ...groups.editors]),
    ).filter((id) => !!id);
   ...
   */
  private async mapFeishuToUsers(
    feishuPeople: FeishuPerson[],
  ): Promise<string[]> {
