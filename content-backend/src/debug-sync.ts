...
async function debugSync() {
    ...
    try {
        ...
        const user = await userRepository.findOne({ where: { tenantId: tenant.id } });
        if (!user) {
            log('❌ No user found for tenant');
            throw new Error('User not found');
        }
        log(`✅ User Found: ${user.name}`, { role: user.role, feishuId: user.feishuId });
        ...
        try {
            ...
            const result = await articleSyncService.syncArticleToFeishu(mockArticle, tenant);
            log('✅ Article Sync Result:', result);
        } catch (error) {
            log('❌ Article Sync Failed', { error: error.message, stack: error.stack });
        }
