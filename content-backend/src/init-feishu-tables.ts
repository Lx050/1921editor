...
async function initFeishuTables() {
    ...
    const appId = tenant.feishuAppId || configService.get<string>('FEISHU_APP_ID');
    const appSecret = tenant.feishuAppSecret || configService.get<string>('FEISHU_APP_SECRET');
    ...
    try {
        ...
        if (userTable) {
            console.log(`   ✅ 找到现有表: [${userTable.name}] (${userTable.table_id})`);
                    ...
                    table: {
                        ...
                        fields: [
                            { field_name: '姓名', type: 1 },
                            {
                                field_name: '岗位',
                                type: 3,
                                property: { options: [{ name: 'ADMIN' }, { name: 'EDITOR' }, { name: 'COPYWRITER' }, { name: 'PLANNER' }] }
                            },
            ...
            if (createUserRes.code !== 0) throw new Error(`创建人员表失败: ${createUserRes.msg}`);
        ...
        if (articleTable) {
            console.log(`   ✅ 找到现有表: [${articleTable.name}] (${articleTable.table_id})`);
                    ...
                    table: {
                        ...
                        fields: [
                            { field_name: '标题', type: 1 },
                            {
                                field_name: '状态',
                                type: 3,
                                property: { options: [{ name: 'DRAFT' }, { name: 'PARSED' }, { name: 'ADJUSTED' }, { name: 'PUBLISHED' }] }
                            },
            ...
            if (createArticleRes.code !== 0) throw new Error(`创建文章表失败: ${createArticleRes.msg}`);
