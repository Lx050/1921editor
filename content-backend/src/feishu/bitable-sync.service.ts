...
export class BitableSyncService {
  ...
  async syncArticleToTable(article: Article, tenant: Tenant): Promise<boolean> {
    ...
    try {
      ...
      // 状态映射：已同步 / 已编辑
      const feishuStatus =
        article.status === ArticleStatus.PUBLISHED ? '已同步' : '已编辑';
      ...
      this.logger.debug(
        `[BitableSyncService] 同步字段详情: ${JSON.stringify(fields)}`,
      );

      // 检查之前是否已经同步过 (通过系统ID反查)
      this.logger.debug(
        `[BitableSyncService] 尝试通过系统ID反查记录: ${article.id}`,
      );
      const existingRecord = await this.feishuSdk.findRecordByField(
        appToken,
        tableId,
      ...
      if (existingRecord) {
        ...
        this.logger.debug(
          `[BitableSyncService] 发现已存在记录: ${finalRecordId}`,
        );
        await this.feishuSdk.updateBitableRecord(
          appToken,
          tableId,
        ...
        // 创建新记录
        const recordId = await this.feishuSdk.createBitableRecord(
          appToken,
          tableId,
          fields,
        );
