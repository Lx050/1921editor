...
class ExchangeAuthDto {
    @ApiProperty({ description: '微信提供的授权码 (auth_code)', example: 'QUERY_AUTH_CODE' })
    code: string;
}
...
export class WechatController {
    ...
    @ApiOperation({ summary: '微信服务器 VerifyTicket 推送接口', description: '接收微信第三方平台每 10 分钟推送一次的 ticket' })
    ...
    async receiveTicket(@Body() data: any, @Req() req: any) {
        ...
        if (!ticket && typeof data === 'string') {
            const match = data.match(/<ComponentVerifyTicket><!\[CDATA\[(.*)\]\]><\/ComponentVerifyTicket>/)
     ...
     */
    @Post('dev-set-ticket')
    @ApiOperation({ summary: '手动更新 VerifyTicket (开发用)', description: '如果微信推送无法到达本地，可手动调用此接口设置 ticket' })
    ...
    @ApiBearerAuth()
    @ApiOperation({ summary: '生成预授权码', description: '获取用于引导用户扫码授权的 pre_auth_code' })
    ...
    @UseGuards(JwtAuthGuard)
    @ApiBearerAuth()
    @ApiOperation({ summary: '通过 auth_code 换取令牌', description: '在用户扫码授权回调后调用，完成租户与公众号的关联' })
    @ApiResponse({ status: 201, description: '授权成功' })
    async exchangeAuth(@Req() req, @Body() body: ExchangeAuthDto) {
        try {
            const tenantId = req.user.tenantId;
            const result = await this.wechatService.exchangeAuthCode(tenantId, body.code);
    ...
    @UseGuards(JwtAuthGuard)
    @ApiBearerAuth()
    @ApiOperation({ summary: '获取已授权账号列表', description: '获取当前租户下所有已授权的微信公众号及其基本信息' })
    ...
    @UseGuards(JwtAuthGuard)
    @ApiBearerAuth()
    @ApiOperation({ summary: '检查微信授权状态 (旧接口兼容)', description: '获取当前租户下所有已授权的微信账号' })
    ...
    @UseGuards(JwtAuthGuard)
    @ApiBearerAuth()
    @ApiOperation({ summary: '上传图片到微信临时素材', description: '为文章发布准备图片资源' })
    ...
    @UseGuards(JwtAuthGuard)
    @ApiBearerAuth()
    @ApiOperation({ summary: '创建微信草稿', description: '将文章内容同步到微信公众号草稿箱' })
    @ApiResponse({ status: 201, description: '草稿创建成功' })
    async createDraft(@Body() articleData: any, @Req() req) {
        try {
            const tenantId = req.user.tenantId;
            const result = await this.wechatService.createDraft(tenantId, articleData);
    ...
    @UseGuards(JwtAuthGuard)
    @ApiBearerAuth()
    @ApiOperation({ summary: '解约/移除微信账号授权', description: '从本系统中逻辑移除对该公众号的授权，不再展示' })
    ...
    @Get('image-proxy')
    @ApiOperation({ summary: '微信图片代理', description: '代理访问微信图片以绕过防盗链' })
