...
export class FeishuWebhookService {
  ...
  private async handleBitableEvent(body: any) {
    ...
    const tenant = tenants.find((t) => {
      const s = t.settings || {};
      return (
        (s.userTable?.appToken === app_token &&
          s.userTable?.tableId === table_id) ||
        (s.articleTable?.appToken === app_token &&
          s.articleTable?.tableId === table_id)
      );
    });

    if (!tenant) {
      this.logger.warn(
        `找不到配置了表格 ${app_token}/${table_id} 的租户，忽略该事件`,
      );
      return;
    }
    ...
    // 2. 判断是哪张表并分发
    if (
      settings.articleTable?.appToken === app_token &&
      settings.articleTable?.tableId === table_id
    ) {
      this.logger.log('分发到文章表同步处理器');
      await this.articleSyncService.syncArticleFromFeishu(event, tenant);
    } else if (
      settings.userTable?.appToken === app_token &&
      settings.userTable?.tableId === table_id
    ) {
      this.logger.log(
        '分发到用户表同步处理器 (单向同步中，记录变更已记录，暂不反向同步)',
      );
    }
  }
}
