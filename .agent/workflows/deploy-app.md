...
3. **上传到服务器**
   ...
   ```powershell
   scp -o StrictHostKeyChecking=no -i "c:/Users/Lx050/.ssh/editor135.pem" paiban.tar.gz root@101.42.158.32:/root/
   ```
...
6. **查看实时日志 (可选)**
   ...
   ```powershell
   ssh -o StrictHostKeyChecking=no -i "c:/Users/Lx050/.ssh/editor135.pem" root@101.42.158.32 'docker logs --tail 50 -f paiban-backend-1'
   ```
